### ORIGINAL: https://github.com/DJArmstrong/TransitSOM/blob/master/TransitSOM_Release/selfsom.py
...
class SimpleSOMMapper(Mapper):
    ...
    def __repr__(self):
        ...
        if not s[-1] == '(':
            s += ' '
        s += 'kshape=%s, niter=%i, learning_rate=%f, iradius=%f)' % (str(tuple(self.kshape)), self.niter, self.lrate,self.radius)
        return s
